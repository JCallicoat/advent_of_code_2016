...
# also indicates the position within the password to fill. You still look for hashes
...
while '^' in password:
    ...
    if (digest[:5] == prefix):
        try:
            ...
            continue
        if 0 <= index <= 7 and index not in seen:
            password[index] = digest[6]
            seen.append(index)
            decrypting = ''
            for char in password:
                decrypting += random.choice(string.punctuation) if char == '^' else char
            print('got another one! password is {}'.format(decrypting))
print(''.join(password))
