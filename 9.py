...
while index < len(data):
    ...
    if char != '(':
        ...
        index += 1
        continue
    end_marker = data.index(')', index + 1)
    run_length, repeat = map(int, data[index + 1:end_marker].split('x'))
    uncompressed += run_length * repeat
    index = end_marker + 1 + run_length
...
while index < len(data):
    ...
    computation = []
    end_marker = data.index(')', index + 1)
    while data[end_marker + 1] == '(':
        end_marker = data.index(')', end_marker + 1)
    for marker in reversed(data[index + 1:end_marker].split(')(')):
        run_length, repeat = map(int, marker.split('x'))
        if not computation:
            computation = (run_length, repeat)
        else:
            repeated *= repeat
        last_marker = (run_length, repeat)
    run_length, repeat = computation
    uncompressed += run_length * repeat * repeated
    index = end_marker + 1 + computation[0]
    print(end_marker + 1)
    if end_marker + 1 < last_marker[0]:
        uncompressed += run_length * repeat * repeated * last_marker[1]
